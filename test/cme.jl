...
@testset "estimate and statistic" begin
    ...
    for nclasses in (2, 10, 100), nsamples in (10, 50, 100)
        ...
        for targets in (targets_consistent, targets_onlyone)
            ...
            @test test.estimate ≈ calibrationerror(estimator, predictions, targets)

            deviations = deviation.(testpredictions', testtargets', predictions, targets)
...
@testset "consistency" begin
    ...
    for nclasses in (2, 10)
        ...
        for i in eachindex(pvalues_consistent)
            ...
            # define test
            test_consistent = AsymptoticCMETest(estimator, predictions, targets_consistent)
